...
- `codex mcp serve` accepts `--expose-all-tools` to surface the full Codex toolset. Enable auxiliary agents with `--enable-multiagent` (defaults to 2 concurrent agents) and tune capacity with `--max-aux-agents <N>`; this limit is independent of tool exposure. Logs are written to stderr so stdio remains protocol-clean.
