...
use crate::protocol::PlanningOutputEvent;
use crate::protocol::PlanningRequest;
...
/// Minimal two-model planning orchestrator:
/// 1) Planner analyzes the task, explores repo, proposes a plan.
/// 2) Reviewer critiques the plan and suggests improvements.
/// 3) Planner incorporates feedback and prints the final plan; plan is added to history.
async fn spawn_planning_thread(
    sess: Arc<Session>,
    config: Arc<Config>,
    parent_turn_context: Arc<TurnContext>,
    sub_id: String,
    ...
    // Announce planning start
    sess.send_event(Event {
        id: sub_id.clone(),
        msg: EventMsg::EnteredPlanningMode(request.clone()),
    })
    .await;

    // Helper to build a per-turn context for a given model.
    let build_turn_ctx = |model_slug: &str, base_instructions: Option<String>| {
        let provider = parent_turn_context.client.get_provider();
        let auth_manager = parent_turn_context.client.get_auth_manager();
        let family = find_family_for_model(model_slug)
            .unwrap_or_else(|| parent_turn_context.client.get_model_family());
        let mut per_turn_config = (*config).clone();
        per_turn_config.model = model_slug.to_string();
    ...
    let planner_instructions = Some(
        "You are the Planner for this repository. Analyze the user's task, explore files and docs as needed (using tools), and produce a concise, actionable plan.
Include milestones, owners (Planner/Reviewer), and explicit commands only when necessary. Avoid redundant scanning and boilerplate.".to_string(),
    );
    let reviewer_instructions = Some(
        "You are the Reviewer. Critique the plan for completeness, risks, and clarity. Propose concrete fixes and improvements concisely without restating the entire plan.".to_string(),
    );

    let planner_tc = build_turn_ctx(&request.planner_model, planner_instructions.clone());
    let reviewer_tc = build_turn_ctx(&request.reviewer_model, reviewer_instructions.clone());

    // Planner – initial plan
    let mut isolated_history = sess.build_initial_context(planner_tc.as_ref());
    let initial_text = format!(
        "{}\n\n---\n\nTask: {}\n\nStart by outlining a plan. If you need to inspect files, do so briefly and only as needed.",
        planner_instructions.clone().unwrap_or_default(),
        request.task_prompt
    );
    isolated_history
        .push(ResponseInputItem::from(vec![InputItem::Text { text: initial_text }]).into());
    let (planner_plan, _planner_items) = run_isolated_until_final_message(
        &sess,
        planner_tc.as_ref(),
        sub_id.clone(),
        isolated_history,
    ...
    // Reviewer – critique
    let mut reviewer_history = sess.build_initial_context(reviewer_tc.as_ref());
    let reviewer_text = format!(
        "{}\n\n---\n\nPlease review the following plan and propose improvements, fixes, and risks succinctly:\n\n{planner_plan}",
        reviewer_instructions.clone().unwrap_or_default()
    );
    reviewer_history.push(
        ResponseInputItem::from(vec![InputItem::Text {
            text: reviewer_text,
        }])
        .into(),
    );
    let (reviewer_feedback, _review_items) = run_isolated_until_final_message(
        &sess,
        reviewer_tc.as_ref(),
        sub_id.clone(),
        reviewer_history,
    ...
    // Planner – incorporate feedback and finalize
    let mut finalize_history = sess.build_initial_context(planner_tc.as_ref());
    let finalize_text = format!(
        "{}\n\n---\n\nIncorporate the following review feedback and output the final plan (no extra commentary):\n\n{reviewer_feedback}",
        planner_instructions.unwrap_or_default()
    );
    finalize_history.push(
        ResponseInputItem::from(vec![InputItem::Text {
            text: finalize_text,
        }])
        .into(),
    );
    let (final_plan, _final_items) = run_isolated_until_final_message(
        &sess,
        planner_tc.as_ref(),
        sub_id.clone(),
        finalize_history,
    ...
    // Emit final event and record into the main conversation for context
    sess.send_event(Event {
        id: sub_id.clone(),
        msg: EventMsg::ExitedPlanningMode(PlanningOutputEvent {
            plan_text: final_plan.clone(),
        }),
    })
    .await;

    let planning_user_message = format!(
        r#"<user_action>
  <context>User initiated a planning task. Here is the final plan consolidated by planner and reviewer.</context>
...
/// Run an isolated multi-turn conversation until the model produces a final assistant message.
/// Returns (last_assistant_message, all_items_last_turn).
async fn run_isolated_until_final_message(
    sess: &Session,
    turn_ctx: &TurnContext,
    sub_id: String,
    mut isolated_history: Vec<ResponseItem>,
    ...
    loop {
        ...
        .await
        {
            Ok(TurnRunResult {
                ...
                let mut next_responses: Vec<ResponseInputItem> = Vec::new();
                let mut items_vec: Vec<ResponseItem> = Vec::new();
                for p in processed_items.into_iter() {
                    if let Some(resp) = p.response {
                        next_responses.push(resp);
                    }
                    items_vec.push(p.item);
                }
                isolated_history.extend(items_vec.clone());
                if next_responses.is_empty() {
                    let summary = get_last_assistant_message_from_turn(&items_vec)
                        .unwrap_or_else(|| "(no response)".to_string());
                    return (summary, items_vec);
                }
                let response_items: Vec<ResponseItem> =
                    next_responses.into_iter().map(ResponseItem::from).collect();
                isolated_history.extend(response_items);
            }
            Err(e) => {
                return (format!("planning turn failed: {e}"), Vec::new());
            }
        }
    }
}
